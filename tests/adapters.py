...
def get_pretoken_counts(bytes: input_bytes) -> map[str, int]:
...
def replace_sublist(lst, pattern, replacement):
    # lst = [1, 2, 3, 2, 3, 4]
    # pattern = [2, 3]
    # replace = [7]
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    pretoken_indices = {}  #  "hello" -> [104, 101, 108, 108, 111]
    merges: dict[tuple[int, int], int] = {}  # (token_id_1, token_id_2) -> new_token_id
    vocab: dict[int, bytes] = {}  # token_id -> token_bytes (str -> encode)
    ...
    target_num_merges = vocab_size - len(special_tokens) - len(vocab)

    assert (
        target_num_merges > 0
    ), "Target vocab size must be larger than the number of special tokens"

    for i in range(target_num_merges):
        # (index1, index2) -> count
        ...
        if max_id12 is not None:
            ...
            for s, token_id_list in pretoken_indices.items():
                pretoken_indices[s] = replace_sublist(
                    token_id_list,
